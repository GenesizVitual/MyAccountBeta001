...
class PembelianPenjuanlan
{
    ...
    public static function jurnal($object, $transaksi){
        if(!empty($object->tgl_pembelian)){
            ...
            $jurnal = Jurnal::where('id_pembelian',$object->id)->first();
            if(empty($jurnal)){
                $jurnal = new Jurnal();
            }
            ...
            $jurnal = Jurnal::where('id_penjualan', $object->id)->first();
            if(empty($jurnal)){
                $jurnal = new Jurnal();
            }
    ...
    public static function pembelian($object){
        $data_jurnal = self::jurnal($object, "Pembelian Barang Dagang ".$object->LinkToProduk->nama_barang);
        if($data_jurnal){
            //Akun Pembelian
            $jurnal_transaksi_persediaan = JurnalTransaksiAkun::where('jurnal_id', $data_jurnal->id)->where('akun_transaksi_id', 23)->first();

            if(empty($jurnal_transaksi_persediaan)){
            ...
            if($jurnal_transaksi_persediaan->save()){
                ...
                //PPN Masukan
                $jurnal_transaksi_ppn_masukan = JurnalTransaksiAkun::where('jurnal_id', $data_jurnal->id)->where('akun_transaksi_id', 26)->first();

                if(empty($jurnal_transaksi_ppn_masukan)){
                ...
                if($jurnal_transaksi_ppn_masukan->save()){
                    ...
                    $jurnal_transaksi_kas = JurnalTransaksiAkun::where('jurnal_id', $data_jurnal->id)->where('akun_transaksi_id', self::$akun_tunai_atau_kredit)->first();

                    if(empty($jurnal_transaksi_kas)){
    ...
    public static function penjualan($object){
        ...
        $data_jurnal = self::jurnal($object, "Penjualan Barang Dagang ".$object->LinkToProduk->nama_barang);
        if($data_jurnal){

            //Kas Atau Utang dagang
            $jurnal_transaksi_persediaan = JurnalTransaksiAkun::where('jurnal_id', $data_jurnal->id)->where('akun_transaksi_id', self::$akun_tunai_atau_kredit)->first();
            ...
            $jurnal_transaksi_persediaan->jum_debet = ($object->kwantitas * $object->harga)+ $object->jumlah_pajak;
            $jurnal_transaksi_persediaan->jum_kredit = 0;
            $jurnal_transaksi_persediaan->id_bisnis = $object->id_bisnis;
            ...
            if($jurnal_transaksi_persediaan->save()){
                ...
                //PPN Keluaran
                $jurnal_transaksi_ppn_keluaran = JurnalTransaksiAkun::where('jurnal_id', $data_jurnal->id)->where('akun_transaksi_id', 27)->first();

                if(empty($jurnal_transaksi_ppn_keluaran)){
                ...
                $jurnal_transaksi_ppn_keluaran->tgl_jurnal = $object->tgl_penjualan;
                $jurnal_transaksi_ppn_keluaran->kategori_jurnal = 1;

                if($jurnal_transaksi_ppn_keluaran->save()){
                    //Pendapatan
                    $jurnal_transaksi_kas = JurnalTransaksiAkun::where('jurnal_id', $data_jurnal->id)->where('akun_transaksi_id', 28)->first();
                    ...
                    $jurnal_transaksi_kas->jurnal_id = $data_jurnal->id;
                    $jurnal_transaksi_kas->akun_transaksi_id = 28;
                    $jurnal_transaksi_kas->jum_debet = 0;
                    $jurnal_transaksi_kas->jum_kredit = ($object->kwantitas * $object->harga);
                    $jurnal_transaksi_kas->id_bisnis = $object->id_bisnis;
